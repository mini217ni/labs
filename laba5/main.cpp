...
public:
    ...
    Pair operator+(const Pair& other) const { return Pair(first + second, other.first + other.second); }
...
public:
    ...
    Complex operator-(const Complex& other) const { return Complex(first - other.first, second - other.second); }
...
int main() {
    ...
    for (auto& c : arr) { cout << "  " << c.toString() << endl; }

    // принцип подстановки (базовый указатель на производный объект)
    Pair* ptr = new Complex(5, -2);
    cout << "\nПринцип подстановки (Pair* -> Complex): " << ptr->toString() << endl;
    delete ptr;
